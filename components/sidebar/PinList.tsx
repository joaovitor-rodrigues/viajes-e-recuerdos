...
export default function PinList() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
      ...
      <div style={{ padding: '10px 12px 8px', borderBottom: `1px solid ${DIV}` }}>
        <input
          ...
          style={{
            ...
            border: '1px solid rgba(201,72,91,0.12)',
            background: 'rgba(201,72,91,0.03)', color: TEXT, fontSize: 12, outline: 'none',
      ...
      {/* Group toggle */}
      <div style={{ padding: '8px 12px', display: 'flex', gap: 6, borderBottom: `1px solid ${DIV}` }}>
        {(['nenhum', 'país', 'ano'] as GroupMode[]).map((m) => (
          <button
            ...
            style={{
              ...
              background: groupMode === m ? 'var(--primary-color, #C9485B)' : 'rgba(201,72,91,0.07)',
              color: groupMode === m ? '#fff' : MUTED,
        ...
        {filtered.length === 0 ? (
          ...
          sortedGroupKeys.map((key) => (
            <div key={key}>
              {groupMode !== 'nenhum' && (
                <div style={{
                  padding: '6px 16px 4px', fontSize: 10, color: MUTED,
